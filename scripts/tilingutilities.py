    ...
    for d in range(0, depth, d_size - overlap):
        for r in range(0, rows, r_size - overlap):
            for c in range(0, cols, c_size - overlap):
                ...
                tiles.append((tile, d_start, r_start, c_start))
...
def stitch_volume(tiles: List[Tuple[NDArray[T], int, int, int]], 
    ...
    result = np.zeros(original_shape, dtype=tiles[0][0].dtype)
    counts = np.zeros(original_shape, dtype=np.int32)

    # Process each tile
    for tile, d_start, r_start, c_start in tiles:
