...
def generate_torchinfo_summary(model, input_size, output_path):
    ...
    try:
        ...
        ax.text(0.05, 0.95, str(model_summary), transform=ax.transAxes, 
                fontfamily='monospace', fontsize=6, verticalalignment='top')
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
        ax.axis('off')
...
def main():
            ...
            for config_name, config in configs.items():
                ...
                for depth in config['depths']:
                    ...
                    try:
                        ...
                        print(f"Created {model_type}: {model.__class__.__name__} (depth={depth})")
                        ...
                        # Generate summary
                        summary_path = output_dir / f"{model_type}_{config_name}_d{depth}_summary"
                        total_visualizations += 1
                        if generate_torchinfo_summary(model, input_size, summary_path):
                            successful_visualizations += 1
                        
                        # Generate computation graph
                        graph_path = output_dir / f"{model_type}_{config_name}_d{depth}_graph"
                        total_visualizations += 1
                        if generate_torchviz_graph(model, input_tensor, graph_path):
                            successful_visualizations += 1
                            
                    except Exception as e:
                        print(f"✗ Failed to process {model_type} {config_name} depth {depth}: {e}")
                        import traceback
                        traceback.print_exc()
                        continue
    ...
    if total_visualizations > 0:
        print(f"Success rate: {successful_visualizations/total_visualizations*100:.1f}%")
    print(f"Output directory: {output_dir}")
