...
def create_architecture_comparison_plots(csv_path, output_dir="./plots"):
    ...
    architecture_name_mapping = {
        ...
        "Anisotropicunet3D-D3-Hk(1-3-3)-Dk(1-2-2)": "AnisoUNet(2D)",
        "Anisotropicunet3D-D3-Hk(1-3-3)-Dk(1-2-2)-Usenet": "USENet(2D)",
        "Anisotropicunet3D-D3-Hk(1-3-3)-Dk(1-2-2)-Usenet-Sf16": "USEnet(2D)",
        "Anisotropicunet3D-D3-Hk(3-3-3)-Dk(1-2-2)-Usenet": "USENet(3D)",
        "Anisotropicunet3D-D3-Hk(3-3-3)-Dk(1-2-2)-Usenet-Sf16": "USENet(3D)",
        "Anisotropicunet3D-D3-Hk(3-3-3)-Dk(1-2-2)-Usenet-Sf16": "USENet(3D)",
        'Anisotropicunet3D-D3-Hk(3-3-3)-Dk(1-2-2)_Usenet': "USENet(3D)",
        "Anisotropicunet3D-D3-Hk(1-3-3)-Dk(1-2-2)-Csam": "CSAM(2D)",
        'Anisotropicunet3D-D3-Hk(1_ 3_ 3)-Dk(1_ 2_ 2)-Csam': "CSAM(2D)",
        'Anisotropicunet3D-D3-Hk(3_ 3_ 3)-Dk(1_ 2_ 2)-Csam': "CSAM(3D)",
        "Anisotropicunet3D-D3-Hk(3-3-3)-Dk(1-2-2)-Csam": "CSAM(3D)",
        "AnisotropicUNet3DCSAM-d2-hk(3-3-3)-dk(1-2-2)": "CSAM(3D)",
    }
    ...
    for metric_col, metric_name in available_metrics.items():
        ...
        for arch in arch_labels:
            short_name = arch.replace("AnisotropicUNet3D-", "").replace(
                "-hk(3-3-3)-dk(1-2-2)", ""
            )
...
def create_comprehensive_summary(df, available_metrics, output_dir):
    ...
    fig.suptitle(
        "Architecture Performance Summary - All Metrics", fontsize=16, fontweight="bold"
    )
    ...
    for metric_col, metric_name in available_metrics.items():
            ...
            metric_rankings.append(
                {
                    ...
                    .replace("AnisotropicUNet3D-", "")
                    .replace("-hk(3-3-3)-dk(1-2-2)", ""),
        ...
        for i, ranking in enumerate(metric_rankings[:5]):  # Top 5
            print(
                f"  {i + 1:2d}. {ranking['Architecture']:<15} = {ranking['Mean']:.4f} ± {ranking['Std']:.4f}"
            )


def main():
    parser = argparse.ArgumentParser(
        description="Create architecture comparison plots from quad-based CSV."
    )
    parser.add_argument("csv_path", help="Path to quad-based aggregated CSV file")
    parser.add_argument(
        "-o", "--output-dir", default="./plots", help="Output directory for plots"
    )
