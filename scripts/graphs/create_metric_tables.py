...
def create_heatmap(pivot_table, metric_name, output_dir, metric_col):
    ...
    # Create heatmap
    im = plt.imshow(
        pivot_table.values, cmap="YlOrRd", aspect="auto", vmin=0.2, vmax=0.7
    )

    # Set labels
    plt.xticks(
        range(len(pivot_table.columns)), [f"{col}px" for col in pivot_table.columns]
    )
    plt.yticks(
...
def main():
    parser = argparse.ArgumentParser(
        description="Create 2D metric tables from aggregated CSV."
    )
    parser.add_argument("csv_path", help="Path to aggregated CSV file")
    parser.add_argument(
        "-o", "--output-dir", default="./tables", help="Output directory"
    )
