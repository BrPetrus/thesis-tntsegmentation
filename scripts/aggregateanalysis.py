...
def aggregate_csv_files(directory_path, output_path, overlap_filter=None):
    ...
    if overlap_filter is not None:
        ...
        print(f"After filtering Overlap == {overlap_filter}: {filtered_df.shape} (removed {initial_count - len(filtered_df)} rows)")
    ...
    if overlap_filter is not None:
        ...
        sample_counts = filtered_df.groupby(['Architecture', 'Quad']).size().reset_index(name='Count')
        for _, row in sample_counts.iterrows():
            print(f"  Arch={row['Architecture']} | Quad={row['Quad']}: {row['Count']} samples")
    else:
        print(f"\nSample counts per (Architecture, Quad, Overlap):")
        sample_counts = filtered_df.groupby(['Architecture', 'Quad', 'Overlap']).size().reset_index(name='Count')
        for _, row in sample_counts.iterrows():
            print(f"  Arch={row['Architecture']} | Quad={row['Quad']} | Overlap={row['Overlap']}: {row['Count']} samples")
...
def aggregate_csv_files_across_quads(directory_path, output_path, overlap_filter=None):
    ...
    if overlap_filter is not None:
        ...
        sample_counts = filtered_df.groupby(['Architecture']).size().reset_index(name='Count')
        for _, row in sample_counts.iterrows():
            print(f"  Arch={row['Architecture']}: {row['Count']} samples (across all quads)")
    else:
        print(f"\nSample counts per (Architecture, Overlap):")
        sample_counts = filtered_df.groupby(['Architecture', 'Overlap']).size().reset_index(name='Count')
        for _, row in sample_counts.iterrows():
            print(f"  Arch={row['Architecture']} | Overlap={row['Overlap']}: {row['Count']} samples (across all quads)")
  ...
  python aggregateanalysis.py ./results -o arch_20px.csv --overlap 20 --across-quads
    ...
    try:
        if args.across_quads:
            print("Using CROSS-QUADRANT aggregation (Architecture performance across all quadrants)")
            result_df = aggregate_csv_files_across_quads(args.directory, args.output, args.overlap)
            analysis_script = "archcomparisonquad.py"
        else:
            print("Using PER-QUADRANT aggregation (Architecture performance within each quadrant)")
        ...
        if result_df is not None:
            ...
            overlap_msg = f" with overlap={args.overlap}" if args.overlap is not None else ""
