...
class TunnelMappingResult:
    ...
    multimatched_predictions: Dict[int, List[Tuple[int, float]]]  # pred_label -> [(gt_label, score), ...]
...
def analyze_tunnel_mappings(gt_labeled: NDArray[np.uint8], 
                          labeled_prediction: NDArray[np.uint8], 
                          recall_threshold: float) -> TunnelMappingResult:
    """
    ...
    multimatched_pred, unmatched_labels, unmatched_predictions = find_unmatched_and_multi_mapped(
...
def calculate_all_metrics(gt_labeled: NDArray[np.uint8], 
                         labeled_prediction: NDArray[np.uint8],
                         binary_gt: NDArray[np.bool_],
                         binary_pred: NDArray[np.bool_],
                         mapping_result: TunnelMappingResult) -> Tuple[QualityMetrics, QualityMetrics, QualityMetrics, QualityMetrics]:
    ...
    matched_metrics = calculate_matched_metrics(gt_labeled, labeled_prediction, mapping_result.mapping)
    
    # Metrics for clean 1-to-1 mappings only
    clean_metrics = calculate_matched_metrics(gt_labeled, labeled_prediction, mapping_result.clean_mapping)
...
def detect_tunnels(prediction: NDArray[np.float32], 
                  gt_labeled: NDArray[np.uint8], 
                  image: NDArray, 
                  config: PostprocessConfig, 
                  output_folder: Path, 
                  visualise: bool = False) -> TunnelDetectionResult:
    """
    ...
    mapping_result = analyze_tunnel_mappings(gt_labeled, labeled_prediction, config.recall_threshold)
    
    # Calculate all metrics
    overall_metrics, matched_metrics, clean_metrics, tunnel_metrics = calculate_all_metrics(
        gt_labeled, labeled_prediction, binary_gt, binary_filtered_pred, mapping_result
    )

    # Print basic metrics
    print("Overall metrics:")
    print(f"prec={overall_metrics.prec:.3f}, recall={overall_metrics.recall:.3f}, "
          f"jaccard={overall_metrics.jaccard:.3f}, dice={overall_metrics.dice:.3f}")

    # Visualization
    if visualise:
        visualise_mapping_results(gt_labeled, image, output_folder, labeled_prediction, 
                                binary_filtered_pred, binary_gt, mapping_result)
...
def visualise_mapping_results(gt_labeled, image, output_folder, labeled_prediction, 
    ...
    visualise_matching(
        gt_labeled, image, output_folder, labeled_prediction, binary_filtered_pred, binary_gt,
        mapping_result.mapping, mapping_result.multimatched_predictions, 
        mapping_result.unmatched_gt_labels, mapping_result.unmatched_predictions
    )
...
def print_detailed_results(result: TunnelDetectionResult):
    ...
    print(f"TP: {result.metrics_all_matches.tp}, FP: {result.metrics_all_matches.fp}, FN: {result.metrics_all_matches.fn}")
    ...
    print(f"TP: {result.metrics_one_on_one.tp}, FP: {result.metrics_one_on_one.fp}, FN: {result.metrics_one_on_one.fn}")
...
if __name__ == "__main__":
    ...
    result = detect_tunnels(prediction, label, img, config, 
                           output_folder=output_folder / 'ontunnels', visualise=True)
